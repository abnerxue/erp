		...
		<view v-for="(item,index) in list" :key='index' class="goos_lists">
			<view class="space_between  ">
				...
				<view class="_state  align_items_center">
					<text v-if="item.payStatus == 1 || item.payStatus == 2" class="goods_state">待付款</text>
					<text v-if="item.logisticsStatus == 2" class="goods_state">待收货</text>
					<text v-if="item.logisticsStatus == 3" class="goods_state">待评价</text>
					<text v-if="item.logisticsStatus == 4" class="goods_state">交易完成</text>
					<image v-if="item.logisticsStatus == 4" class="delete_image" src="/static/order/delete.png"></image>
				</view>
			</view>
			<view @click="goToPage(item)" class="sigle_goos_list">
				...
				<view class="goods_description flex_row">
					<view class="title_size flex_column">
						<text class="goods_title">{{item.goods[0].title}}</text>
<!--						<text class="goods_size">{{item.size}}</text>-->
					</view>
					<view class="money_amount  flex_column">
						<text class="goods_money">￥{{item.totalPrice}}</text>
						<text class="goods_amount">X 1</text> </view>
				</view>
			</view>
			...
			<view class="state_btn">
				<text v-if="item.payStatus == 1 || item.payStatus == 2" class="_btn">去支付</text>
				<text v-if="item.payStatus == 3" class="_btn">再次购买</text>
				<text v-if="item.logisticsStatus == 3" class="_btn">去评价</text>
				<text v-if="item.logisticsStatus == 2" class="_btn">去收货</text>
			</view>
