<template>
	<view class="goos_container">
		<view v-for="(item,index) in list" :key='index' class="goos_lists">
			...
			<view class="bottom_amount_money"> <text class="_amount">共1件商品</text> <text class="need_pay" v-if="item.payStatus == 1 || item.payStatus == 2">
					需付款 ：</text> <text class="need_pay" v-if="item.payStatus == 3"> 实付款 ：</text>
				<text class="_money">￥{{item.totalPrice}}</text> </view>
