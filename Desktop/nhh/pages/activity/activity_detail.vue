...
<script>
	...
	export default {
		...
		onLoad(option) {
			let that = this;
			if (option.activity == 1) { //团购
				var url = 'api/group/detail.html';
			} else if (option.activity == 2) { //秒杀
				var url = '/api/seckill/detail.html';
			} else if (option.activity == 3) { //砍价
				var url = '/api/bargain/detail.html';
			} else if (option.activity == 4) { //积分换购
				var url = '/api/score/detail.html';
			}
			var id = option.id;
